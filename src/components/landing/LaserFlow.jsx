...
const LaserFlow = ({ className, style, horizontalBeamOffset = 0.1, verticalBeamOffset = 0.0, flowSpeed = 0.35, verticalSizing = 2.0, horizontalSizing = 0.5, fogIntensity = 0.45, fogScale = 0.3, color = '#FF79C6', dpr }) => {
  const mountRef = useRef(null), uniformsRef = useRef(null);
  useEffect(() => {
    ...
    const rgb = (hex => { let c = hex.replace('#',''); if (c.length===3) c = c.split('').map(x => x+x).join(''); const n = parseInt(c,16)||0xffffff; return [((n>>16)&255)/255, ((n>>8)&255)/255, (n&255)/255]; })(color); uniforms.uColor.value.set(...rgb);
    const clock = new THREE.Clock(); let raf; const animate = () => { raf = requestAnimationFrame(animate); uniforms.iTime.value = clock.getElapsedTime(); uniforms.uFade.value = Math.min(1, uniforms.uFade.value + 0.02); renderer.render(scene, camera); }; animate();
